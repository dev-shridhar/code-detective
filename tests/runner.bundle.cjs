...
async function main() {
  ...
  await import_web_tree_sitter.default.init({
    locateFile: (scriptPath) => import_path.default.resolve(wasmDir, scriptPath)
  });
  ...
  for (const tc of testCases) {
    ...
    const src = import_fs.default.readFileSync(filePath, "utf-8");
    const tree = parser.parse(src);
...
WARNING: class ${clsName} not found in ${tc.file}`);
        ...
        const find = (node2, name2) => {
          if ((node2.type === "function_definition" || node2.type === "async_function_definition" || node2.type === "class_definition") && node2.childForFieldName("name")?.text === name2) return node2;
          for (const child of node2.namedChildren) {
